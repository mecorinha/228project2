...
What if we wanted to `filter()` by a categorical variable like pokemon type (`type1`) instead of a numerical variable like Health points (`hp`)? We can do that! Even better, we can do both at the same time! (Since the `=` operator is used for *assignment*, we use the `==` operator to *test for equality*)
